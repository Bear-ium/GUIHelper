...
-- Base creator
local function createInstance(className, a, b, c)
	local name, parent, props

	if typeof(a) == "string" and typeof(b) == "Instance" then
		name, parent, props = a, b, c
	elseif typeof(a) == "Instance" and (typeof(b) == "string" or b == nil) then
		parent, name, props = a, b, c
	elseif typeof(a) == "Instance" and typeof(b) == "table" and c == nil then
		parent, props = a, b
	else
	...
	if props then applyProps(instance, props) end
	return instance
end

-- GUI Containers
function GUI.CreateScreenGUI(playerGUI, GUIName, props)
	local ScreenGUI = createInstance("ScreenGui", GUIName, playerGUI, props)
	ScreenGUI:SetAttribute("CustomGUI", true)
	return ScreenGUI
end
